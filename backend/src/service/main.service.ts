...
import { AircraftClassInstance } from "../model/aircraftclass";
dotenv.config({ path: path.join(__dirname, "../../.env") });

class mainServiceClass {
  create_flight_details = async (bodys: any) => {
    console.log("model", bodys?.AIRCRAFT_ICAO);
    const classchecker = await AircraftClassInstance.findOne({
      where: {
        AIRCRAFT_ICAO: bodys?.AIRCRAFT_ICAO,
      },
    });

    console.log("result", classchecker?.AIRCRAFT_CLASS);
    console.log("result", classchecker?.AIRCRAFT_CLASS);
    console.log("result", classchecker?.AIRCRAFT_CLASS);
    console.log("result", classchecker?.AIRCRAFT_CLASS);
    console.log("result", classchecker?.AIRCRAFT_CLASS);
    ...
    if (classchecker) {
      var air_class = classchecker?.AIRCRAFT_CLASS;
  ...
  postAircraftClass = async (bodys: any) => {
    const result = await AircraftClassInstance.create({
      AIRCRAFT_CLASS: bodys?.AIRCRAFT_CLASS,
      AIRCRAFT_NAME: bodys?.AIRCRAFT_NAME,
    });
    return result;
  };

  getAirCraftClass = async (req: Request) => {
    const result = await FlightDetailsInstance.findAll();
    return result;
  };
}
